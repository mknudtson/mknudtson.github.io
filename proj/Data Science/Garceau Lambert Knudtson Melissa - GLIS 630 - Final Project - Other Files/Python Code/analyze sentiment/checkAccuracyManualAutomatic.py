...
for row in csv_f:  
  ...
  if int(row[8]) > 0:
      ...
      if sent_manuInt > sent_autoInt:
          ...
          ###if (sent_manuInt - sent_autoInt) > 1:
              #if the difference exceeds +/-1, add 1 to counter
              counter = counter + 1
      elif sent_autoInt > sent_manuInt:
          #if automatic > manual, find out what the difference is
          ###if (sent_autoInt - sent_manuInt) > 1:
              #if the difference exceeds +/-1, add 1 the counter
              counter = counter + 1    
              
print("Number of automatically labelled records that are inaccurately labelled: " + str(counter))    
print("Percentage accuracy: " + str(((353 - counter) / 353) * 100))
